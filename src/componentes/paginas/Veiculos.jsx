...
function Veiculos() {
  ...
  const [select, setSelected] = useState(null)
   ...
   const selecionarVeiculo = (indice) => {
    ...
    setSelected({value: veiculos[indice].idCliente, label : veiculos[indice].idCliente.nome})
}
...
const alterar = () => {
  ...
  const remover = () => {
    fetch('http://localhost:8080/veiculos/'+objVeiculo.idVeiculo, {
      ...
      .then(retorno_convertido => {
        if (retorno_convertido.message !== undefined) {
          ...
          //indice
          let indice = vetorTemporario.findIndex((c) =>{
            return c.id === objVeiculo.idVeiculo;
          } );
    ...
    return(
        <div>
            <FormularioVeiculo selecionar={selecionarVeiculo} botao={btnCadastrar} select={select} setSelected={setSelected}
            eventoTeclado={aoDigitar} obj={objVeiculo} setObj={setObjVeiculo} cadastrar={cadastrar} 
            alterar={alterar} remover={remover} cancelar={limparFormulario}/>
